...
### Added
- Schema-drift tolerant PostgreSQL writes — the writer now inserts only the columns that exist in the target table, skipping source-only columns (with a warning) instead of failing the whole insert. Makes satellite/cross-environment clones work when source and target schemas differ slightly
