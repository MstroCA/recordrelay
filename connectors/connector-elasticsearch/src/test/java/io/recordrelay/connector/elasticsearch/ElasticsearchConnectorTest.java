...
/** Unit tests for {@link ElasticsearchConnector}. */
class ElasticsearchConnectorTest {

  private final ElasticsearchConnector connector = new ElasticsearchConnector();

  @Test
  void connectorIdIsElasticsearch() {
    assertThat(connector.connectorId()).isEqualTo("elasticsearch");
  }

  @Test
  void supportsElasticsearchProfiles() {
    assertThat(connector.supports(profile(DatabaseType.ELASTICSEARCH))).isTrue();
  }
