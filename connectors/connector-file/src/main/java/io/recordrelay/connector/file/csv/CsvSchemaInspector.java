...
/** Reads header row of a CSV file to infer column names. */
public final class CsvSchemaInspector implements SchemaInspector {
  ...
  public List<ColumnMeta> inspectColumns(ConnectionProfile profile, TableRef table)
    ...
    try (var reader = new FileReader(profile.database());
      ...
      for (var col : headers) {
        result.add(
            new ColumnMeta(col, "STRING", true, false, false, ordinal.getAndIncrement(), null));
      }
      return List.copyOf(result);
    } catch (IOException e) {
      throw new ConnectorException("Failed to inspect CSV headers: " + e.getMessage(), e);
    }
  }
}
