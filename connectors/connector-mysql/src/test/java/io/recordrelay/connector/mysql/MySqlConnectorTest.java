...
class MySqlConnectorTest {
  ...
  @Test
  void connectorIdIsMysql() {
    assertThat(connector.connectorId()).isEqualTo("mysql");
  }

  @Test
  void supportsMysqlProfiles() {
    assertThat(connector.supports(profile(DatabaseType.MYSQL))).isTrue();
  }

  @Test
  void supportsMariaDbProfiles() {
    assertThat(connector.supports(profile(DatabaseType.MARIADB))).isTrue();
  }
  ...
  private ConnectionProfile profile(DatabaseType type) {
    return new ConnectionProfile(
        "id", "test", "env", type, "localhost", 3306, "db", new Credentials("u", "p"), Map.of());
  }
}
