...
/** Unit tests for {@link OracleConnector}. */
class OracleConnectorTest {

  private final OracleConnector connector = new OracleConnector();

  @Test
  void connectorIdIsOracle() {
    assertThat(connector.connectorId()).isEqualTo("oracle");
  }

  @Test
  void supportsOracleProfiles() {
    assertThat(connector.supports(profile(DatabaseType.ORACLE))).isTrue();
  }
  ...
  @Test
  void listDatabasesReturnsSingleServiceEntry() throws Exception {
    var p =
        new ConnectionProfile(
            ...
            1521,
            "ORCLPDB1",
            new Credentials("sys", "password"),
            Map.of());
    var dbs = connector.listDatabases(p);
    assertThat(dbs).hasSize(1);
    assertThat(dbs.get(0).name()).isEqualTo("ORCLPDB1");
  }
