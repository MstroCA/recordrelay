...
public final class SqlServerConnector implements ContextProviderPort {
  ...
  public boolean supports(ConnectionProfile profile) {
    return profile.type() == DatabaseType.SQLSERVER;
  }

  @Override
  public void testConnection(ConnectionProfile profile) throws ConnectorException {
    try (var ds = SqlServerDataSourceFactory.create(profile);
  ...
  public List<DatabaseRef> listDatabases(ConnectionProfile profile) throws ConnectorException {
    try (var ds = SqlServerDataSourceFactory.create(profile);
      ...
      while (rs.next()) {
        result.add(new DatabaseRef(rs.getString("name"), DatabaseType.SQLSERVER));
      }
  ...
  public SchemaInspector schemaInspector() {
    return new SqlServerSchemaInspector();
  }

  @Override
  public RecordReader createReader() {
    return new SqlServerRecordReader();
  }

  @Override
  public RecordWriter createWriter() {
    return new SqlServerRecordWriter();
  }
}
