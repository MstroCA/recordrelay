...
/** Unit tests for {@link CassandraConnector}. */
class CassandraConnectorTest {

  private final CassandraConnector connector = new CassandraConnector();

  @Test
  void connectorIdIsCassandra() {
    assertThat(connector.connectorId()).isEqualTo("cassandra");
  }

  @Test
  void supportsCassandraProfiles() {
    assertThat(connector.supports(profile(DatabaseType.CASSANDRA))).isTrue();
  }
  ...
  private ConnectionProfile profile(DatabaseType type) {
    return new ConnectionProfile(
        ...
        9042,
        "mykeyspace",
        new Credentials("cassandra", "cassandra"),
        Map.of());
  }
}
