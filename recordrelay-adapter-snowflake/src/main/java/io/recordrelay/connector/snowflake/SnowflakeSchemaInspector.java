...
/**
 * {@link SchemaInspector} adapter for Snowflake.
 *
 * <p>Uses {@code information_schema} views which Snowflake exposes in each database. Primary key
 * detection uses {@code table_constraints} and {@code key_column_usage}, both ANSI-standard.
 */
public final class SnowflakeSchemaInspector implements SchemaInspector {
  ...
  private static final String LIST_COLUMNS_SQL =
      "SELECT c.column_name, c.data_type, c.is_nullable, c.column_default, "
          ...
          + "WHERE c.table_catalog ILIKE ? AND c.table_schema ILIKE ? AND c.table_name ILIKE ? "
          + "ORDER BY c.ordinal_position";

  @Override
  public List<TableRef> listTables(ConnectionProfile profile, DatabaseRef database)
      throws ConnectorException {
    String schemaOverride = profile.properties().get("schema");
    ...
    try (var ds = DataSourceFactory.create(profile);
      ...
      stmt.setString(1, database.name().toUpperCase(Locale.ROOT));
  ...
  public List<ColumnMeta> inspectColumns(ConnectionProfile profile, TableRef table)
    ...
    try (var ds = DataSourceFactory.create(profile);
      ...
      stmt.setString(2, table.schemaName().isBlank() ? "PUBLIC" : table.schemaName());
  ...
  private static List<TableRef> loadUserTables(java.sql.DatabaseMetaData meta, DatabaseRef database)
    ...
    try (var rs = meta.getTables(null, null, "%", new String[] {"TABLE"})) {
      while (rs.next()) {
        ...
        tables.add(
            new TableRef(database, schema != null ? schema : "", rs.getString("TABLE_NAME")));
      }
    }
    return tables;
  }
